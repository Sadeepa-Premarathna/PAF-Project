...
public class AuthController {
    ...
    public ResponseEntity<AuthResponse> handleOAuthCallback(
        ...
        OAuth2UserInfo userInfo = googleOAuthClient.exchangeCodeAndGetUserInfo(
    ...
    private String extractRefreshTokenCookie(HttpServletRequest request) {
        if (request.getCookies() == null) {
            throw new com.smartcampus.auth.exception.UnauthorizedException("No refresh token cookie");
        }
        return Arrays.stream(request.getCookies())
                .filter(c -> REFRESH_TOKEN_COOKIE.equals(c.getName()))
                .map(Cookie::getValue)
                .findFirst()
                .orElseThrow(() -> new com.smartcampus.auth.exception.UnauthorizedException(
